...
public class HttpFaceRecog {
	...
	public void clear() {
		try {
			int existGroupNum = httpRequests.infoGetGroupList().getJSONArray("group").length();
			for (int i = 0; i < existGroupNum; i++) {
				String gName = httpRequests.infoGetGroupList().getJSONArray("group").getJSONObject(i).getString("group_name");
				System.out.println(httpRequests.groupDelete(new PostParameters().setGroupName(gName)));
			}
			int existpersonNum = httpRequests.infoGetPersonList().getJSONArray("person").length();
			for (int i = 0; i < existpersonNum; i++) {
				String pName = httpRequests.infoGetPersonList().getJSONArray("person").getJSONObject(0).getString("person_name");
				System.out.println(httpRequests.personDelete(new PostParameters().setPersonName(pName)));
			}
			...
			for (int k = 0; k < groupN; k++) {
				...
				while (iter.hasNext()) {
					...
					for (int j = 0; j < ids.size(); j++) {
						...
						result = httpRequests.detectionDetect(new PostParameters().setImg(new File(f)));
						System.out.println(result);
						System.out.println(httpRequests.personAddFace(new PostParameters().setPersonName(personName).setFaceId(result.getJSONArray("face").getJSONObject(0).getString("face_id"))));
					}
					personList.add(personName);
				}
				System.out.println(httpRequests.groupAddPerson(new PostParameters().setGroupName("group" + k).setPersonName(personList)));
				System.out.println("\ntrain/Identify");
	...
	public String recognize(String file) {
		...
		try {
			...
			for (int k = 0; k < existGroupNum; k++) {
				String groupName = "group_" + k;
				String ff = file;
				JSONObject recog = httpRequests.recognitionIdentify(new PostParameters().setGroupName(groupName).setImg(new File(ff)));
				int num = recog.getJSONArray("face").getJSONObject(0).getJSONArray("candidate").length();
				if (num > 1) {
					double winnerConf = recog.getJSONArray("face").getJSONObject(0).getJSONArray("candidate").getJSONObject(0).getDouble("confidence");
					String winnerName = recog.getJSONArray("face").getJSONObject(0).getJSONArray("candidate").getJSONObject(0).getString("person_name");
					for (int a = 1; a < num; a++) {
						double confidence = recog.getJSONArray("face").getJSONObject(0).getJSONArray("candidate").getJSONObject(a).getDouble("confidence");
						String personName = recog.getJSONArray("face").getJSONObject(0).getJSONArray("candidate").getJSONObject(a).getString("person_name");
					...
					System.out.println("Result:" + winnerName + "" + winnerConf);
